"""Module for simulating and analyzing dice throws."""
...
def throw_dice(num_faces: int = 6) -> int:
    ...
    Returns:
        int: Result of the die throw.

    Raises:
        ValueError: If number of faces is less than 1.
    """
    if num_faces < 1:
...
def throw_n_dice(num_dice: int, num_faces: int = 6) -> List[int]:
    ...
    Args:
        num_dice: Number of dice to throw.
        num_faces: Number of faces on each die.

    Returns:
        List[int]: Results of all dice throws.

    Raises:
        ValueError: If number of dice is less than 1.
    """
    if num_dice < 1:
...
def measure_frequency_of_outcome(outcome: int, list_of_outcomes: List[int]) -> None:
    ...
    Args:
        outcome: The outcome to measure.
        list_of_outcomes: List of all outcomes to analyze.
    """
...
def estimate_probability_of_dice_faces(n_throws: int, num_faces: int = 6) -> None:
    ...
    Args:
        n_throws: Number of throws to simulate.
        num_faces: Number of faces on each die.

    Raises:
        ValueError: If number of throws is less than 1.
    """
    if n_throws < 1:
